# Flexible Geo / Engineering MLOps

> Reproducible, configurable machine-learning workflows for engineering and geoscience research.

Flexible Geo / Engineering MLOps is a configurable machine-learning research workflow for tabular, geotechnical, geological, spatial, and related engineering datasets. Experimental targets, feature sets, grouping strategies, class thresholds, model families, input datasets, and output directories can be controlled primarily through YAML profiles, reducing the need to modify the Python source code between studies.

The main graphical interface is `soil_mlops_gui.py`. The canonical active configuration file is `params.yaml`.
...
Because the original research dataset cannot be fully distributed, the public example `samples.csv` contains only a minimal sanitized record. The workflow can be run with a user-supplied dataset that follows the configuration defined in the active YAML profile.
## 9. Command reference (`install.txt`)
...
## 13. Repository and archival guidance

Generated models, reports, processed data, caches, virtual environments, MLflow artifacts, and private/raw research datasets should remain excluded through `.gitignore`. Commit the source code, dependency specification, sanitized YAML profiles, and—when permitted—a small non-sensitive example dataset. For a Zenodo release, archive a tagged GitHub release and include the exact configuration files needed to reproduce the reported experiments.
